...
import { convertMarkdownToHtml, convertMarkdownToPlainText } from './entryConverter';

describe('EntryConverter', () => {
  describe('convertMarkdownToHtml', () => {
    test('Markdown text is converted to intended HTML', async () => {
      // Given
      const markdown = await readFile(path.resolve(__dirname, './__fixtures__/i-entered-kua.md'), 'utf-8');
      const expected = await readFile(path.resolve(__dirname, './__fixtures__/i-entered-kua.html'), 'utf-8');
  ...
  describe('convertMarkdownToPlainText', () => {
    test('Markdown text is converted to intended plain text', async () => {
      // Given
      const markdown = await readFile(path.resolve(__dirname, './__fixtures__/i-entered-kua.md'), 'utf-8');
      const expected = await readFile(path.resolve(__dirname, './__fixtures__/i-entered-kua.txt'), 'utf-8');
